...
import (
	...
	"github.com/cryptix-network/cryptix-graph-inspector/processing/database/model"
	...
	"github.com/cryptix-network/cryptix-graph-inspector/processing/infrastructure/tools"
	"github.com/cryptix-network/cryptix-graph-inspector/processing/processing/batch"
...
type Processing struct {
	...
	cryptixd  *cryptixdPackage.Cryptixd
...
func NewProcessing(config *configPackage.Config,
	database *databasePackage.Database, cryptixd *cryptixdPackage.Cryptixd) (*Processing, error) {

	appConfig := &model.AppConfig{
		ID:                true,
		CryptixdVersion:   version.Version(),
...
func (p *Processing) initConsensusEventsHandler() {
	go func() {
		for {
			consensusEvent, ok := <-p.cryptixd.Domain().ConsensusEventsChannel()
			if !ok {
				return
			}
...
func (p *Processing) ResyncDatabase() error {
	...
	return p.database.RunInTransaction(func(databaseTransaction *pg.Tx) error {
		...
		pruningPointHash, err := p.cryptixd.Domain().Consensus().PruningPoint()
		if err != nil {
			return err
		}

		pruningPointBlock, err := p.cryptixd.Domain().Consensus().GetBlock(pruningPointHash)
		if err != nil {
			return err
		}
		...
		headersSelectedTip, err := p.cryptixd.Domain().Consensus().GetHeadersSelectedTip()
		if err != nil {
			return err
		}
		log.Infof("Load node blocks")
		hashesBetweenPruningPointAndHeadersSelectedTip, _, err := p.cryptixd.Domain().Consensus().GetHashesBetween(pruningPointHash, headersSelectedTip, 0)
		if err != nil {
			return err
		}
		...
		for i := startIndex; i < len(hashesBetweenPruningPointAndHeadersSelectedTip); i++ {
			...
			block, err := p.cryptixd.Domain().Consensus().GetBlockEvenIfHeaderOnly(blockHash)
			if err != nil {
				return err
			}
...
func (p *Processing) resyncVirtualSelectedParentChain(databaseTransaction *pg.Tx, withDependencies bool) error {
	...
	if len(virtualSelectedParentChain.Added) > 0 {
		...
		virtualSelectedParentBlock, err := p.cryptixd.Domain().Consensus().GetBlock(virtualSelectedParentHash)
		if err != nil {
			return err
		}
...
func (p *Processing) processBlockAndDependencies(databaseTransaction *pg.Tx, hash *externalapi.DomainHash,
	...
	batch := batch.New(p.database, p.cryptixd, pruningBlock)
...
func (p *Processing) processBlock(databaseTransaction *pg.Tx, block *externalapi.DomainBlock) error {
	...
	blockInfo, err := p.cryptixd.Domain().Consensus().GetBlockInfo(blockHash)
	if err != nil {
		// enhanced error description
	...
	blockGHOSTDAGData, err := p.cryptixd.BlockGHOSTDAGData(blockHash)
	if err != nil {
...
func (p *Processing) processVirtualChange(databaseTransaction *pg.Tx, blockInsertionResult *externalapi.VirtualChangeSet, withDependencies bool) error {
	...
	for _, addedBlockHash := range addedBlockHashes {
		addedBlockGHOSTDAGData, err := p.cryptixd.BlockGHOSTDAGData(addedBlockHash)
		if err != nil {
...
func (p *Processing) getBlocksDAAScores(databaseTransaction *pg.Tx, blockHashes []*externalapi.DomainHash) (map[uint64]uint64, error) {
	...
	for _, blockHash := range blockHashes {
		block, err := p.cryptixd.Domain().Consensus().GetBlockHeader(blockHash)
		if err != nil {
			return nil, err
		}
		blockID, err := p.database.BlockIDByHash(databaseTransaction, blockHash)
