...
func New(database *databasePackage.Database, cryptixd *cryptixdPackage.Cryptixd, prunningBlock *externalapi.DomainBlock) *Batch {
	batch := &Batch{
		database:      database,
		cryptixd:      cryptixd,
...
func (b *Batch) CollectDirectDependencies(databaseTransaction *pg.Tx, hash *externalapi.DomainHash, block *externalapi.DomainBlock) error {
	...
	for _, parentHash := range parentHashes {
		...
		if !parentExists {
			parentBlock, err := b.cryptixd.Domain().Consensus().GetBlockEvenIfHeaderOnly(parentHash)
			if err != nil {
				// We ignore the `block not found` cryptixd error.
				// In this case the parent is out the node scope so we have no way
				// to include it in the batch
				if !errors.Is(err, database.ErrNotFound) {
					return err
				} else {
					log.Warnf("Parent %s for block %s not found by cryptixd domain consensus; the missing dependency is ignored", parentHash, hash)
				}
			} else {
				b.Add(parentHash, parentBlock)
				log.Warnf("Parent %s for block %s found by cryptixd domain consensus; the missing dependency is registered for processing", parentHash, hash)
			}
		}
	}
	return nil
