...
import (
	...
	"github.com/cryptix-network/cryptix-graph-inspector/processing/database/utils/lrucache"
...
func (db *Database) StoreAppConfig(databaseTransaction *pg.Tx, appConfig *model.AppConfig) error {
	appConfig.ID = true
	_, err := databaseTransaction.Model(appConfig).OnConflict("(id) DO UPDATE SET cryptixd_version = EXCLUDED.cryptixd_version, processing_version = EXCLUDED.processing_version, network = EXCLUDED.network").Insert()
	if err != nil {
		return err
	}
	return nil
